...
export function PrioritizationQueue() {
  const [activeTab, setActiveTab] = useState<'queue' | 'investment_map'>('queue');
  ...
  const investmentFloors = useMemo(() => {
    return [
      {
        id: 'dev_prod',
  ...
  const hoveredFloor = useMemo(() => investmentFloors.find(f => f.id === hoveredFloorId) || null, [investmentFloors, hoveredFloorId]);
    ...
    <div className="flex flex-col gap-[18px] animate-[fadeIn_0.2s_ease]">
      ...
      <div className="glass-card rounded-xl p-4 sm:p-[18px_20px]">
        <div className="flex gap-1 bg-[var(--bg4)] p-1 rounded-lg w-fit flex-wrap mb-4">
          ...
          <button
            onClick={() => setActiveTab('investment_map')}
            className={`px-4 py-1.5 rounded-md text-xs font-medium transition-colors whitespace-nowrap ${
              activeTab === 'investment_map' ? 'bg-[var(--bg3)] text-[var(--text)]' : 'text-[var(--text3)] hover:text-[var(--text2)]'
            }`}
          >
        ...
        {activeTab === 'investment_map' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="glass-card rounded-xl p-4 sm:p-[18px_20px] border border-[var(--border)]">
                <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-4">
                  <div className="min-w-0">
                    ...
                    <div className="text-[12px] text-[var(--text-dim)] mt-1">Passe o mouse sobre um andar para ver o total estimado e acessar os projetos.</div>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      setActiveTab('queue');
                      setQueueListTab('to_prioritize');
                    }}
                    className="px-3 py-2 rounded-md text-xs font-semibold bg-[var(--bg4)] text-[var(--text2)] hover:text-[var(--text)] transition-colors"
                  >
                    Ver projetos na Fila
                  </button>
                </div>
                ...
                <div className="relative flex justify-center py-8">
                  ...
                  >
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-[86%] h-[22px] rounded-t-[16px] bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.08)] shadow-[0_12px_40px_rgba(0,0,0,0.35)]" />
                    <div className="rounded-[18px] overflow-hidden border border-[rgba(255,255,255,0.10)] shadow-[0_20px_80px_rgba(0,0,0,0.45)] bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))]">
                      <div className="h-[18px] bg-[rgba(0,0,0,0.18)]" />
                      <div className="px-4 py-4">
                        <div className="grid grid-cols-1 gap-2">
                          {[...investmentFloors].reverse().map((f, idx) => {
                            const isTop = idx === 0;
                            ...
                            return (
                              <div
                                ...
                                className={`group relative rounded-xl border transition-all duration-200 cursor-pointer ${
                                  isHovered
                                    ? 'border-[rgba(56,189,248,0.55)] bg-[rgba(56,189,248,0.12)] shadow-[0_10px_30px_rgba(56,189,248,0.10)]'
                                    : 'border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)] hover:border-[rgba(56,189,248,0.35)] hover:bg-[rgba(56,189,248,0.07)]'
                                }`}
                                style={{
                                  transform: isHovered ? 'translateY(-1px) scale(1.01)' : 'translateY(0px) scale(1)',
                                }}
                              >
                                <div className={`absolute left-0 top-0 bottom-0 w-[6px] ${isHovered ? 'bg-[rgba(56,189,248,0.65)]' : 'bg-[rgba(255,255,255,0.06)]'} ${isTop ? 'rounded-tl-xl' : ''} rounded-bl-xl`} />
                                <div className="flex items-center justify-between gap-3 p-3">
                                  <div className="min-w-0">
                                    <div className="text-[11px] font-bold text-[var(--text)] truncate">{f.name}</div>
                                    <div className="text-[11px] text-[var(--text-dim)] mt-0.5">Investimento: <span className="text-[var(--red)] font-semibold">{fmtCurrency(f.investment)}</span></div>
                                  </div>
                                  <div className="shrink-0 text-right">
                                    <div className="text-[10px] text-[var(--text-dim)] uppercase tracking-[0.07em] font-bold">ROI Est.</div>
                                    <div className="text-[12px] font-extrabold text-[var(--green)]">{fmtCurrency(f.roiTotal)}</div>
                                  </div>
                                </div>
                                <div className="px-3 pb-3">
                                  <div className="grid grid-cols-3 gap-2">
                                    <div className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-[rgba(0,0,0,0.18)] p-2">
                                      ...
                                      <div className="text-[11px] font-bold text-[var(--receita)] mt-0.5">{fmtCurrency(f.receita)}</div>
                                    </div>
                                    <div className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-[rgba(0,0,0,0.18)] p-2">
                                      <div className="text-[9px] text-[var(--text-dim)] font-bold uppercase tracking-[0.07em]">Saving</div>
                                      <div className="text-[11px] font-bold text-[var(--saving)] mt-0.5">{fmtCurrency(f.saving)}</div>
                                    </div>
                                    <div className="rounded-lg border border-[rgba(255,255,255,0.08)] bg-[rgba(0,0,0,0.18)] p-2">
                                      <div className="text-[9px] text-[var(--text-dim)] font-bold uppercase tracking-[0.07em]">C. Evitado</div>
                                      <div className="text-[11px] font-bold text-[var(--custo-evitado)] mt-0.5">{fmtCurrency(f.custoEvitado)}</div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                      ...
                      <div className="h-[22px] bg-[rgba(0,0,0,0.22)]" />
                    </div>

                    <div className="mt-4 flex items-center justify-center">
                      <div className="w-[92%] h-[18px] rounded-full bg-[rgba(0,0,0,0.35)] blur-[10px]" />
                    </div>

                    {hoveredFloor && tooltipPos && (
                      <div
                        className="absolute z-20 pointer-events-none"
                        style={{
                          left: 0,
                          top: 0,
                          transform: `translate(${Math.min(Math.max(tooltipPos.x + 18, 10), 360)}px, ${Math.min(Math.max(tooltipPos.y + 18, 10), 420)}px)`
                        }}
                      >
                        <div className="glass-card rounded-xl p-3 border border-[rgba(56,189,248,0.30)] shadow-[0_18px_60px_rgba(0,0,0,0.55)] w-[280px]">
                          ...
                          <div className="mt-3 text-[11px] text-[rgba(56,189,248,0.95)] font-bold underline">Clique em “Ver projetos na Fila”</div>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="glass-card rounded-xl p-4 sm:p-[18px_20px] border border-[var(--border)]">
              <div className="text-[10px] text-[var(--text-mid)] font-bold uppercase tracking-[0.07em]">Insights (Mock)</div>
              <div className="text-[12px] text-[var(--text-dim)] mt-1">Resumo rápido para uma leitura executiva do portfólio por andar.</div>

              <div className="mt-4 space-y-3">
                {investmentFloors.map((f) => (
                  <button
                    key={f.id}
                    type="button"
                    onMouseEnter={() => setHoveredFloorId(f.id)}
                    ...
                    className={`w-full text-left rounded-xl border p-3 transition-all ${
                      hoveredFloorId === f.id
                        ...
                        : 'border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)] hover:border-[rgba(56,189,248,0.30)] hover:bg-[rgba(56,189,248,0.06)]'
                    }`}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="text-[11px] text-[var(--text-dim)] mt-1">
                          Inv.: <span className="font-semibold text-[var(--red)]">{fmtCurrency(f.investment)}</span>
                          <span className="mx-2 text-[rgba(255,255,255,0.18)]">|</span>
                          ROI: <span className="font-semibold text-[var(--green)]">{fmtCurrency(f.roiTotal)}</span>
                        </div>
                      </div>
                      <div className="shrink-0 text-[10px] text-[rgba(56,189,248,0.90)] font-bold underline">Abrir fila</div>
                    </div>
                  </button>
                ))}
              </div>
